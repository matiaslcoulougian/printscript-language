...
class SymbolLexer(private val symbolMap: Map<String, TokenType>, private val stringDelimiters: List<Char> = listOf('\'', '"')) : Lexer {
    ...
    override fun getTokens(input: List<String>, line: Int): List<Token> {
        ...
        for (index in input.indices) {
            for (delimiterIndex in input[index].indices) {
                val char = input[index][delimiterIndex]
                ...
                // If the current character is a symbol, then we add it to the list of tokens
                    ...
                    if (symbolTokenType != null) tokens.add(Token(symbolTokenType, line + index + 1, delimiterIndex))
                }
            }
        }
        return tokens
